...
import { studentPeerApplications, studentPeers, universities, users } from "@/lib/db/schema";
import {
  isAllowedPhotoType,
  isAllowedProofType,
...
const schema = z.object({
  ...
  courseName: z.string().trim().max(2000).optional(),
  currentYearOrBatch: z.string().trim().max(2000).optional(),
  enrollmentStatus: z.enum(["current_student", "alumnus"]),
  homeState: z.string().trim().max(2000).optional(),
  homeCity: z.string().trim().max(2000).optional(),
  languages: z.string().trim().max(2000).optional(),
  phone: z.string().trim().max(2000).optional(),
  message: z.string().trim().max(2000).optional(),
  website: z.string().trim().max(2000).optional(),
});

export async function submitGuideApplication(formData: FormData, applicant: { id: string; email: string }, headerStore: { get(name: string): string | null }): Promise<JoinAsPeerState> {
  ...
  if (proofFile.size > 10 * 1024 * 1024 || (photoFile instanceof File && photoFile.size > 5 * 1024 * 1024)) return { error: "Use a proof under 10 MB and a photo under 5 MB." };
  ...
  const [existingGuide] = await db.select({ id: studentPeers.id }).from(studentPeers).where(eq(studentPeers.peerUserId, user.id)).limit(1);
  if (existingGuide) return { error: "You already have a guide profile. Open Connect or contact our team for help with your profile." };

  const applicantName = user.name ?? user.email.split("@")[0];
  const applicantEmail = user.email;
  const applicantPhone = data.phone ?? user.phone ?? "";

  if (!/^\+?[\d\s()-]{7,20}$/.test(applicantPhone)) {
    return { error: "Please enter a valid phone number for our team." };
  }
  ...
  const rateLimitError = await consumePublicFormRateLimits(
    [
      { scope: "public:peer_application:user", identifier: applicant.id, limit: 3, windowMs: 60 * 60_000, blockMs: 60 * 60_000 },
      ipAddress
        ? {
