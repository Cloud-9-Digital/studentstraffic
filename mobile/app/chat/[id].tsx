import { supportsNativeCalls, isExpoGo, NATIVE_BUILD_REQUIRED } from "../../src/services/nativeRuntime";
...
import { ConversationSafety } from "../../src/features/chat/ConversationSafety";
...
export default function ChatScreen() {
  ...
  const booking = bookings.find((item) => item.bookingId === data?.conversation.bookingId && item.bookingStatus === "accepted");
  ...
  const contactWarning = sendError?.includes("contact details") ?? false;
  ...
  async function send() {
    const text = body.trim(); if (!text || !data?.conversation.canMessage || sending) return;
    setSendError(null);
    ...
    try { await mobileClient.sendConversationMessage(conversationId, text, `${conversationId}-${Date.now()}`); await refetch(); setOptimistic((items) => items.filter((item) => item.id !== temporaryId)); await client.invalidateQueries({ queryKey: ["conversations"] }); setTimeout(() => scrollToLatest(true), 100); }
    catch (error) { setSendError(error instanceof Error ? error.message : "Unable to send message. Please try again."); setOptimistic((items) => items.filter((item) => item.id !== temporaryId)); setBody(text); }
  ...
  return <View style={s.root}>
    <SafeAreaView edges={["top"]} style={s.safe}><View style={s.header}>
      ...
      {data?.conversation && <ConversationSafety conversationId={conversationId} blockedByMe={data.conversation.blockedByMe} onChanged={() => void refetch()} />}
      {booking && supportsNativeCalls && data?.conversation.canCall ? <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); startCall(booking.bookingId, booking.fullName, booking.universityName); }} disabled={isStarting} style={s.call}><Ionicons name="call" size={18} color="#fff" /></Pressable> : <View style={s.callDisabled}><Ionicons name="call-outline" size={18} color={colors.faint} /></View>}
    </View></SafeAreaView>
    <KeyboardAvoidingView style={s.flex} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      {isLoading ? <View style={s.center}><ActivityIndicator color={colors.primary} /></View> : <>
        ...
        {isExpoGo && <Text style={{ padding: 12, color: "#5d726b" }}>{NATIVE_BUILD_REQUIRED}</Text>}
        {sendError && <View accessibilityRole="alert" accessibilityLiveRegion="polite" style={s.warning}><Ionicons name={contactWarning ? "shield-checkmark-outline" : "alert-circle-outline"} size={22} color={colors.accent} /><View style={s.warningCopy}><Text style={s.warningTitle}>{contactWarning ? "Keep your contact details private" : "Message not sent"}</Text><Text style={s.warningText}>{sendError}</Text></View></View>}
        <View style={[s.composer, { paddingBottom: Math.max(insets.bottom, 10) }]}><TextInput value={body} onChangeText={(value) => { setBody(value); setSendError(null); }} onFocus={() => setTimeout(() => scrollToLatest(true), 80)} editable={data?.conversation.canMessage ?? false} accessibilityLabel="Message" maxLength={2000} placeholder={data?.conversation.canMessage ? "Message…" : "Conversation unavailable"} placeholderTextColor={colors.faint} style={s.input} multiline /><Pressable onPress={send} disabled={!body.trim() || sending || !data?.conversation.canMessage} accessibilityLabel="Send message" style={[s.send, (!body.trim() || sending) && s.sendDisabled]}>{sending ? <ActivityIndicator size="small" color="#fff" /> : <Ionicons name="send" size={18} color="#fff" />}</Pressable></View>
      </>}
...
const s = StyleSheet.create({
  warning: { flexDirection: "row", gap: 10, marginHorizontal: 12, marginBottom: 8, padding: 14, borderRadius: 14, backgroundColor: colors.coralSoft },
  warningCopy: { flex: 1 },
  warningTitle: { fontFamily: "PlusJakartaSans-SemiBold", fontSize: 13, color: colors.accentStrong, marginBottom: 5 },
  warningText: { fontFamily: "PlusJakartaSans-Regular", fontSize: 12, lineHeight: 19, color: colors.ink },
