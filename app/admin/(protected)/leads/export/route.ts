...
export async function GET() {
  ...
  const rows = await db
    .select({
      ...
      needsFmgeSession: leads.needsFmgeSession,
      documentUrl: leads.documentUrl,
      documentType: leads.documentType,
  ...
  const csvLines = [
    ...
    ...rows.map((row) =>
      [
        ...
        row.budgetRange,
        row.needsFmgeSession === true ? "Yes" : row.needsFmgeSession === false ? "No" : "",
        row.documentUrl,
        row.documentType,
        row.userState,
        row.courseSlug,
